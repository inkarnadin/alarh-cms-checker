...
public class CMSDeterminant implements Determinant<CMSType, Destination> {
    ...
    public Map<CMSType, Destination> define(Params params) {
        ...
        wpTransmit.ifPresent(destination -> {
            result.put(CMSType.WORDPRESS, destination);
        });
        ...
        jmTransmit.ifPresent(destination -> {
            result.put(CMSType.JOOMLA, destination);
        });
        ...
        yiiTransmit.ifPresent(destination -> {
            result.put(CMSType.YII, destination);
        });
        ...
        dleTransmit.ifPresent(destination -> {
            result.put(CMSType.DATALIFE_ENGINE, destination);
        });
